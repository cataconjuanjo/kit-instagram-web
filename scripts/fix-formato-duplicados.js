'use strict'
// fix-formato-duplicados.js
// Generaliza fix-vino-magnum-duplicados.js a TODOS los FORMAT_WORDS.
//
// Por cada par (vino_base sin formato, vino_con_formato, misma bodega):
//   1. Re-parentar vino_anada de vino_con_formato → vino_base (id intacto)
//   2. Borrar dedup_candidato del par (SIN crear dedup_decision)
//   3. Borrar vino_con_formato — solo tras verificar count(vino_anada)=0, id individual
//
// --dry-run por defecto. --apply para escribir.

const fs   = require('fs')
const path = require('path')
const { createClient } = require('@supabase/supabase-js')

// ── Entorno ───────────────────────────────────────────────────────────────────

function loadEnv() {
  const env = {}
  for (const f of ['.env.local', '.env.vercel']) {
    if (!fs.existsSync(f)) continue
    for (const line of fs.readFileSync(f, 'utf8').split(/\r?\n/)) {
      const m = line.match(/^\s*([^#=\s]+)\s*=\s*(.*)\s*$/)
      if (!m) continue
      env[m[1]] = m[2].replace(/^['"]|['"]$/g, '')
    }
  }
  return { ...env, ...process.env }
}

// ── Normalización (copiada literalmente de insertar-candidatos-formato.js) ────

function normTexto(v) {
  if (!v) return ''
  return v
    .normalize('NFD').replace(/[̀-ͯ]/g, '')
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
}

// Palabras de formato que el script de build NO elimina del nombre (deuda bloque 9)
const FORMAT_WORDS = [
  'doble magnum', 'double magnum', 'media botella', 'half bottle',
  'matusalem', 'methuselah', 'balthazar', 'salmanazar', 'rehoboam',
  'jeroboam', 'imperial', 'magnum',
]

function tieneFormatoEmbebido(normNombre) {
  return FORMAT_WORDS.some(fw => normNombre.includes(fw))
}

function quitarFormato(normNombre) {
  let s = normNombre
  // Orden importante: primero los compuestos (doble magnum antes que magnum)
  for (const fw of FORMAT_WORDS) s = s.replace(fw, ' ')
  return s.replace(/\s+/g, ' ').trim()
}

// ── Supabase helpers ──────────────────────────────────────────────────────────

async function fetchAll(sb, table, select, extra) {
  const all = []
  for (let offset = 0; ; offset += 1000) {
    let q = sb.from(table).select(select).range(offset, offset + 999)
    if (extra) q = extra(q)
    const { data, error } = await q
    if (error) throw new Error(`${table}: ${error.message}`)
    all.push(...(data || []))
    if (!data || data.length < 1000) break
  }
  return all
}

// ── Main ──────────────────────────────────────────────────────────────────────

async function main() {
  const DRY_RUN = !process.argv.includes('--apply')
  console.log(DRY_RUN ? '[DRY RUN] Pasar --apply para escribir en BD.\n'
                       : '[APPLY] Modo escritura activo.\n')

  const env = loadEnv()
  const sb  = createClient(
    env.NEXT_PUBLIC_SUPABASE_URL,
    env.SUPABASE_SERVICE_ROLE_KEY,
    { auth: { persistSession: false } },
  )

  // ── 1. Cargar todos los vinos ─────────────────────────────────────────────
  console.log('Cargando vinos…')
  const vinos = await fetchAll(sb, 'vino', 'id, nombre, bodega_id')
  console.log(`  ${vinos.length} vinos\n`)

  // Separar con-formato / sin-formato, indexados por bodega
  const conFormato          = []
  const sinFormatoPorBodega = new Map()

  for (const v of vinos) {
    if (!v.bodega_id) continue
    const norm = normTexto(v.nombre)
    if (tieneFormatoEmbebido(norm)) {
      conFormato.push({ ...v, _norm: norm })
    } else {
      if (!sinFormatoPorBodega.has(v.bodega_id)) sinFormatoPorBodega.set(v.bodega_id, [])
      sinFormatoPorBodega.get(v.bodega_id).push({ ...v, _norm: norm })
    }
  }

  // ── 2. Identificar pares ──────────────────────────────────────────────────
  const pares = []
  for (const vm of conFormato) {
    const candidatos = sinFormatoPorBodega.get(vm.bodega_id) || []
    const base       = candidatos.find(v => v._norm === quitarFormato(vm._norm))
    if (!base) continue
    const formatoDetectado = FORMAT_WORDS.find(fw => vm._norm.includes(fw)) || '?'
    pares.push({ vinoBase: base, vinoConFormato: vm, formatoDetectado })
  }

  const bodegasN = new Set(pares.map(p => p.vinoBase.bodega_id)).size
  console.log(`Pares identificados: ${pares.length} en ${bodegasN} bodegas`)

  if (pares.length === 0) {
    console.log('Sin correcciones necesarias.')
    return
  }

  // Desglose por palabra de formato
  const porFormato = new Map()
  for (const { formatoDetectado } of pares) {
    porFormato.set(formatoDetectado, (porFormato.get(formatoDetectado) || 0) + 1)
  }
  console.log('\nDesglose por formato:')
  for (const [fw, n] of [...porFormato.entries()].sort((a, b) => b[1] - a[1])) {
    console.log(`  ${fw.padEnd(22)} ${n}`)
  }

  // ── 3. Cargar vino_anada para todos los vinos implicados ──────────────────
  const vinoIds = [...new Set(pares.flatMap(p => [p.vinoBase.id, p.vinoConFormato.id]))]
  console.log(`\nCargando vino_anada para ${vinoIds.length} vinos…`)

  const todasVas = []
  for (let i = 0; i < vinoIds.length; i += 100) {
    const rows = await fetchAll(sb, 'vino_anada', 'id, vino_id, anada, formato_ml',
      q => q.in('vino_id', vinoIds.slice(i, i + 100)))
    todasVas.push(...rows)
  }

  const vasByVino = new Map()
  for (const va of todasVas) {
    if (!vasByVino.has(va.vino_id)) vasByVino.set(va.vino_id, [])
    vasByVino.get(va.vino_id).push(va)
  }
  console.log(`  ${todasVas.length} vino_anada cargadas`)

  // ── 4. Cargar dedup_candidato relevantes ──────────────────────────────────
  const vaIds = todasVas.map(v => v.id)
  console.log(`\nCargando dedup_candidato para ${vaIds.length} vino_anada…`)

  const dedupRowsRaw = []
  for (let i = 0; i < vaIds.length; i += 50) {
    const chunk  = vaIds.slice(i, i + 50)
    const filter = chunk.map(id => `vino_anada_a.eq.${id},vino_anada_b.eq.${id}`).join(',')
    const { data, error } = await sb.from('dedup_candidato')
      .select('id, vino_anada_a, vino_anada_b, estado')
      .or(filter)
    if (error) throw new Error(`dedup_candidato: ${error.message}`)
    dedupRowsRaw.push(...(data || []))
  }
  const dedupRows = [...new Map(dedupRowsRaw.map(d => [d.id, d])).values()]
  console.log(`  ${dedupRows.length} dedup_candidato cargados`)

  // ── 5. Planificar operaciones ─────────────────────────────────────────────
  let totalMover      = 0
  let totalConflictos = 0
  let totalDedup      = 0
  let totalParaBorrar = 0

  const ops = []

  for (const { vinoBase, vinoConFormato, formatoDetectado } of pares) {
    const vasBase      = vasByVino.get(vinoBase.id)      || []
    const vasConFmt    = vasByVino.get(vinoConFormato.id) || []

    const clavesBase = new Set(vasBase.map(va => `${va.anada ?? ''}|${va.formato_ml}`))

    const vasMover     = []
    const vasConflicto = []

    for (const va of vasConFmt) {
      if (clavesBase.has(`${va.anada ?? ''}|${va.formato_ml}`)) {
        vasConflicto.push(va)
      } else {
        vasMover.push(va)
        clavesBase.add(`${va.anada ?? ''}|${va.formato_ml}`)
      }
    }

    const vaIdBase   = new Set(vasBase.map(v => v.id))
    const vaIdConFmt = new Set(vasConFmt.map(v => v.id))

    const dedupDelPar = dedupRows.filter(d =>
      (vaIdBase.has(d.vino_anada_a) && vaIdConFmt.has(d.vino_anada_b)) ||
      (vaIdConFmt.has(d.vino_anada_a) && vaIdBase.has(d.vino_anada_b)),
    )

    totalMover      += vasMover.length
    totalConflictos += vasConflicto.length
    totalDedup      += dedupDelPar.length
    if (vasConflicto.length === 0) totalParaBorrar++

    ops.push({
      vinoBase,
      vinoConFormato,
      formatoDetectado,
      vasMover,
      vasConflicto,
      dedupIds: dedupDelPar.map(d => d.id),
    })
  }

  // ── 6. Cargar entradas de carta (tabla vinos) para todas las vino_anada ───
  console.log(`\nCargando entradas de carta (tabla vinos) para ${vaIds.length} vino_anada…`)
  const cartaLineasRaw = []
  for (let i = 0; i < vaIds.length; i += 100) {
    const rows = await fetchAll(
      sb, 'vinos', 'id, nombre, restaurante_id, vino_anada_id',
      q => q.in('vino_anada_id', vaIds.slice(i, i + 100)).not('vino_anada_id', 'is', null),
    )
    cartaLineasRaw.push(...rows)
  }
  const cartaLineas = [...new Map(cartaLineasRaw.map(cl => [cl.id, cl])).values()]
  console.log(`  ${cartaLineas.length} entradas de carta afectadas`)

  // ── 7. Resumen dry-run ────────────────────────────────────────────────────
  const SEP = '═'.repeat(60)
  console.log(`\n${SEP}`)
  console.log('RESUMEN')
  console.log(`  Pares a procesar:             ${pares.length}`)
  console.log(`  vino_anada a re-parentar:     ${totalMover}`)
  console.log(`  vino_anada en conflicto (*):  ${totalConflictos}`)
  console.log(`  vino a borrar:                ${totalParaBorrar}`)
  console.log(`  dedup_candidato a borrar:     ${totalDedup}  (sin crear dedup_decision)`)
  if (totalConflictos > 0)
    console.log(`  (*) misma clave anada+formato_ml ya existe en vinoBase → SKIP`)

  console.log('\n  Desglose por formato:')
  for (const [fw, n] of [...porFormato.entries()].sort((a, b) => b[1] - a[1])) {
    console.log(`    ${fw.padEnd(22)} ${n}`)
  }

  console.log(`\n  ► Entradas de carta de restaurante afectadas: ${cartaLineas.length}`)
  if (cartaLineas.length === 0) {
    console.log('    Ninguna carta de restaurante afectada.')
  } else {
    const W1 = 44, W2 = 38
    console.log(
      '    ' + 'nombre_en_carta'.padEnd(W1) + ' ' +
      'restaurante_id'.padEnd(W2) + ' ' +
      'vino_anada_id',
    )
    console.log('    ' + '-'.repeat(W1 + 1 + W2 + 1 + 36))
    for (const cl of cartaLineas) {
      const nombre = (cl.nombre || '(sin nombre)').substring(0, W1 - 1)
      console.log(
        '    ' + nombre.padEnd(W1) + ' ' +
        (cl.restaurante_id || '(null)').padEnd(W2) + ' ' +
        cl.vino_anada_id,
      )
    }
  }
  console.log(SEP)

  // Listar conflictos
  const opsConConflicto = ops.filter(op => op.vasConflicto.length > 0)
  if (opsConConflicto.length > 0) {
    console.log(`\nConflictos de clave (${opsConConflicto.length} pares — estos vino_anada NO se tocarán):`)
    for (const op of opsConConflicto) {
      console.log(`  "${op.vinoConFormato.nombre}" → "${op.vinoBase.nombre}"`)
      for (const va of op.vasConflicto) {
        console.log(`    ↳ ${va.id}  anada=${va.anada ?? 'null'}  formato_ml=${va.formato_ml}  — SKIP`)
      }
    }
  }

  // Muestra de vino_anada a re-parentar
  console.log('\nMuestra vino_anada a re-parentar (primeras 8):')
  const muestra = ops
    .flatMap(op => op.vasMover.map(va => ({
      ...va,
      _conFmt: op.vinoConFormato.nombre,
      _base:   op.vinoBase.nombre,
    })))
    .slice(0, 8)
  for (const va of muestra) {
    console.log(`  ${va._conFmt.padEnd(30)} → ${va._base.padEnd(30)}  formato_ml=${va.formato_ml}ml  anada=${va.anada ?? 'S/A'}`)
  }

  if (DRY_RUN) {
    console.log('\n[DRY RUN] Sin cambios en BD. Pasar --apply para ejecutar.\n')
    return
  }

  // ── 8. Backup CSV ─────────────────────────────────────────────────────────
  const backupDir  = 'backups'
  if (!fs.existsSync(backupDir)) fs.mkdirSync(backupDir, { recursive: true })
  const ts         = new Date().toISOString().replace(/[:.]/g, '-')
  const backupPath = path.join(backupDir, `fix-formato-${ts}.csv`)

  const esc = v => `"${String(v ?? '').replace(/"/g, '""')}"`
  const csvLines = [
    'formato_detectado,vino_con_formato_id,vino_con_formato_nombre,vino_base_id,vino_base_nombre,bodega_id,vino_anada_id,anada,formato_ml,accion',
  ]
  for (const op of ops) {
    for (const va of op.vasMover)
      csvLines.push([
        esc(op.formatoDetectado),
        op.vinoConFormato.id, esc(op.vinoConFormato.nombre),
        op.vinoBase.id,       esc(op.vinoBase.nombre),
        op.vinoBase.bodega_id, va.id, va.anada ?? '', va.formato_ml,
        'reparentar',
      ].join(','))
    for (const va of op.vasConflicto)
      csvLines.push([
        esc(op.formatoDetectado),
        op.vinoConFormato.id, esc(op.vinoConFormato.nombre),
        op.vinoBase.id,       esc(op.vinoBase.nombre),
        op.vinoBase.bodega_id, va.id, va.anada ?? '', va.formato_ml,
        'conflicto_skip',
      ].join(','))
  }
  fs.writeFileSync(backupPath, csvLines.join('\n'), 'utf8')
  console.log(`\nBackup: ${backupPath}`)

  // ── 9. Aplicar ────────────────────────────────────────────────────────────
  let reparentados  = 0
  let dedupBorrados = 0
  let vinosBorrados = 0
  let errores       = 0

  for (const op of ops) {
    // 9a. Re-parentar vino_anada uno por uno (nunca cambia vino_anada.id)
    for (const va of op.vasMover) {
      const { error } = await sb.from('vino_anada')
        .update({ vino_id: op.vinoBase.id })
        .eq('id', va.id)
      if (error) {
        console.error(`  ERR reparentar vino_anada ${va.id}: ${error.message}`)
        errores++
        continue
      }
      reparentados++
    }

    // 9b. Borrar dedup_candidato del par (sin crear dedup_decision)
    for (const dId of op.dedupIds) {
      const { error } = await sb.from('dedup_candidato').delete().eq('id', dId)
      if (error) {
        console.error(`  ERR dedup_candidato delete ${dId}: ${error.message}`)
        errores++
        continue
      }
      dedupBorrados++
    }

    // 9c. Verificar que no queda ningún vino_anada apuntando a vinoConFormato
    const { count, error: cErr } = await sb
      .from('vino_anada')
      .select('*', { count: 'exact', head: true })
      .eq('vino_id', op.vinoConFormato.id)

    if (cErr) {
      console.error(`  ERR verificar vino_anada de ${op.vinoConFormato.id}: ${cErr.message}`)
      errores++
      continue
    }
    if (count > 0) {
      console.log(`  SKIP borrar "${op.vinoConFormato.nombre}" (${op.vinoConFormato.id}): quedan ${count} vino_anada`)
      continue
    }

    // 9d. Borrar vinoConFormato — id concreto, nunca en bloque
    const { error: delErr } = await sb.from('vino')
      .delete()
      .eq('id', op.vinoConFormato.id)
    if (delErr) {
      console.error(`  ERR borrar vino ${op.vinoConFormato.id}: ${delErr.message}`)
      errores++
      continue
    }
    vinosBorrados++
  }

  // ── 10. Resultado final ───────────────────────────────────────────────────
  console.log(`\n${SEP}`)
  console.log('RESULTADO APPLY')
  console.log(`  vino_anada re-parentadas:  ${reparentados}`)
  console.log(`  dedup_candidato borrados:  ${dedupBorrados}`)
  console.log(`  vino borrados:             ${vinosBorrados}`)
  if (errores > 0) console.log(`  ⚠ ERRORES:                 ${errores}`)
  console.log(SEP + '\n')
}

main().catch(e => { console.error(e.message || e); process.exit(1) })
