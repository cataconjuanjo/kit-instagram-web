...
async function main() {
  ...
  // 1. Tienda
  let { data: tienda } = await sb.from('tiendas').select('id, nombre').eq('slug', SLUG).single()

  if (tienda) {
    console.log(`⚠ Tienda ya existe (${tienda.id}). Actualizando configuración...`)
    const { error } = await sb.from('tiendas').update({
      ...
      color_acento:        '#CFC481',
      font_family:         'Proza Libre',
      background_color:    '#F2EFDD',
      activo:              true,
  ...
  if (!tienda) {
    const { data } = await sb.from('tiendas').select('id').eq('slug', SLUG).single()
    tienda = data
  }

  // 2. Limpiar vinos existentes
  const { error: delErr } = await sb.from('vinos_tienda').delete().eq('tienda_id', tienda.id)
  ...
  const { error: insErr, count } = await sb.from('vinos_tienda').insert(rows)
  if (insErr) { console.error('Error insertando vinos:', insErr.message); process.exit(1) }

  console.log(`✓ ${rows.length} vinos importados`)
  ...
  console.log(`  ${blancos} blancos · ${tintos} tintos · ${espum} espumosos · ${dulces} dulces`)
